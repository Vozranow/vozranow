...
const SessionBookingPage = () => {
  ...
  useEffect(() => {
    const fetchFreshProfile = async () => {
      try {
        ...
        // Using ID here too prevents error spam on load
        toast.error("Could not sync wallet balance.", { id: "wallet_error" });
      } finally {
  ...
  const handleBooking = async () => {
    // 🟢 TOAST FIX: The 'id' prevents stacking. 
    // It will just shake/refresh the existing toast if clicked again.
    
    // 1. Basic Validation
    if (!date || !startTime || !endTime) {
      toast.error("Please select a date and time window.", { id: "validation_error" });
      return;
    }
    ...
    try {
      ...
      // if (startHour < 8) throw new Error("Sessions can only be booked after 8:00 AM.");

      const durationMinutes = (endDateTime - startDateTime) / (1000 * 60);
      if (durationMinutes < 120 || durationMinutes > 180) {
        throw new Error("Preferred window must be between 2 to 3 hours.");
      }
      ...
      if (walletBalance < selectedPlan.price) {
        toast.error("Insufficient Balance. Redirecting...", { id: "balance_error" });
      ...
      if (res.status === 201) {
        toast.success("Request Sent! An admin will assign a listener soon.", { id: "success_msg" });
        navigate("/dashboard");
      }
      ...
      // 🟢 The ID ensures only one error shows at a time
      toast.error(msg, { id: "booking_error" });
    } finally {
    ...
    <div className="min-h-screen bg-[#FDFCF8] pb-12">
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-16 relative z-20 space-y-8">
        ...
        <div className="bg-white rounded-3xl p-6 md:p-8 shadow-xl shadow-gray-200/50">
          ...
          <div className="mt-8 bg-[#F8FAFC] rounded-xl p-4 flex justify-between items-center border border-gray-100">
          ...
          <button 
            ...
            className="w-full mt-6 py-4 bg-[#173F3A] text-white rounded-xl font-bold text-lg hover:bg-[#0F2926] active:scale-[0.98] transition-all disabled:opacity-70 flex items-center justify-center gap-2 shadow-lg shadow-[#173F3A]/20"
          >
